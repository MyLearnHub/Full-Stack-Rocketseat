import close from "../assets/icons/close-black.svg";
import { Button } from "./Button";

type Props = {
  customer: Customer;
  isOpen: boolean;
  onClose: () => void;
};

export function DeleteCustomerModal({ customer, isOpen, onClose }: Props) {
  if (!isOpen) return null;

  return (
    <div className="w-screen h-screen bg-[#00000050] absolute inset-0 flex justify-center items-center z-50">
      <div className="bg-gray-50 w-[358px] rounded-[10px] border border-gray-100 xl:w-[440px]">
        <div className="px-7 py-5 flex justify-between items-center">
          <h2 className="text-md font-bold text-gray-300">Excluir cliente</h2>
          <img
            src={close}
            alt="Ícone de fechar"
            className="cursor-pointer"
            onClick={onClose}
          />
        </div>

        <div className="px-7 pt-7 pb-8 border-y border-gray-100 text-md">
          <p className="mb-5">
            Deseja realmente exluir <strong>{customer.name}</strong>
          </p>
          <p>
            Ao excluir, todos os chamados deste cliente serão removidos e esta
            ação não poderá ser desfeita.
          </p>
        </div>

        <div className="px-7 py-6 flex gap-2">
          <Button variant="secondary">Cancelar</Button>
          <Button>Sim, excluir</Button>
        </div>
      </div>
    </div>
  );
}
